...
> **数据适用范围**：上表及以下要点均在**全局标量控制器**（`pseudo_dt_mode = "global"`，即 RMS 误差范数 + 标量 PI 因子）下测得；引用本表数据时应注明控制器模式。甜点容差点上已做局部/全局对比实测（见 3.5 节）：局部控制器在等熵涡上无收益（慢 1.34–1.45×），完整的局部模式容差扫描仍未做。
...
- 接受判定经 `Blas::amax` 归约 $\max \sigma_{lkj}$；$\min \Delta\tau$ 触底守护经 `Blas::amin` 归约。

### 3.5 局部控制器的实测表现（等熵涡，负结果）

局部步长的收益前提是 **DOF 间伪时间刚性存在显著差异**——这正是 PyFR 提出逐自由度步长的动机（非均匀网格、多尺度问题）。在刚性均匀的问题上，局部化没有可利用的自由度差异，逐 DOF 数组只剩开销。等熵涡（均匀网格上的光滑输运）即属此类，实测确认为负结果。

**实验设置**：等熵涡 32×32、N=2、周期边界、T=2.0、LLF、双精度、OCCA Serial（锁频单核）；`method = ditr_u2r1` + 伪时间 `ssprk321`、`pseudo_rtol = pseudo_atol = 1e-3`（甜点容差）、耦合模式、物理 CFL 10/5；显式 `ssprk3@CFL0.2` 为参考基线；1 次暖机 + 5 次计时。

| 配置                            | 物理步 | 伪迭代总数 | wall 均值±std (s) | local/global | L2        |
| :------------------------------ | :----: | :--------: | :---------------: | :----------: | :-------- |
| ssprk3@CFL0.2（显式基线）       |  535   |     —      |    4.29±0.02      |      —       | 1.138e-03 |
| u2r1_ssprk321@CFL10-**global**  |   11   |    756     |    12.01±0.07     |      —       | 3.056e-03 |
| u2r1_ssprk321@CFL10-**local**   |   11   |    922     |    17.39±0.07     |  **1.448**   | 3.056e-03 |
| u2r1_ssprk321@CFL5-**global**   |   22   |    997     |    16.02±0.12     |      —       | 1.238e-03 |
| u2r1_ssprk321@CFL5-**local**    |   22   |    1134    |    21.52±0.13     |  **1.343**   | 1.237e-03 |

要点：

- **劣势完全对账（两效应相乘）**：伪迭代数增加（CFL10 +22%、CFL5 +14%）× 每伪迭代变贵（~18.9 vs ~15.9 ms，+19%，逐 DOF 数组的读写与 PI 更新开销）≈ 1.448× / 1.343×。
- **机理**：等熵涡为均匀网格上的光滑输运，各 DOF 的伪时间刚性几乎相同，局部化无差异可利用；且接受判据 $\max_{lkj}\sigma_{lkj} < 1$ 比全局 RMS 更保守，逐步伪迭代数波动更大（81~91 vs 稳定的 ~70），推高伪迭代总数。
- **选型建议**：刚性均匀的问题（均匀网格光滑流动）保持默认 `global`；`local` 留给 DOF 间刚性差异显著的场景（非均匀/局部加密网格、边界层、多尺度流动）。ODE 级验证见 `ctest/time/test_time_stepper.cpp` Test 5a 的双速率刚性 ODE——刚性组与温和组步长分化约 6.5×。
- **解质量不受模式影响**：local 的 L2 与 global 一致（CFL5 组 1.237e-03 vs 1.238e-03，舍入级差异）、质量漂移 $\le 7\times10^{-12}$、无 `max_pseudo_steps` 触顶；global 组逐位复现既往甜点扫描的伪迭代数与 L2（回归校验通过，确认实现未扰动 global 路径）。
