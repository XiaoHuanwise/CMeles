...
注意区分两层容差：上式中的 `rtol`/`atol`（`[time_marching]` 同名键）是**外层**伪时间收敛判据；伪步进器自身的**内层**局部误差容差默认取自动启发式（双精度 1e-3 / 单精度 1e-2，松弛以控制迭代数），可经 `[time_marching] pseudo_rtol`/`pseudo_atol` 覆盖——内层过松时外层判据可能在欠收敛状态下提前满足（最终精度由内层容差主导），需要底板级精度时应收紧内层。
...
| 策略                | 配置                        | 适用场景                                                                                    |
| :------------------ | :-------------------------- | :------------------------------------------------------------------------------------------ |
| 自适应-全局（默认） | `pseudo_dt_mode = "global"` | 单一标量步长 + RMS 误差 PI 控制器；行为保守，甜点容差数据已标定（显式自适应文档 §2.4）      |
| 自适应-局部         | `pseudo_dt_mode = "local"`  | 逐模态系数步长（PyFR 风格，显式自适应文档 §3.4–§3.5）；仅当 DOF 间刚性差异显著时有收益（非均匀/局部加密网格、边界层、多尺度）——等熵涡实测为纯开销（慢 1.34–1.45×），刚性均匀的问题保持默认 `global` |
| 固定步长            | `pseudo_dt > 0`             | 调试、简单问题；                                                                            |

局部模式下，解耦 DITR 的两个阶段步进器各自持有独立的逐 DOF 步长数组；耦合模式下单步进器覆盖堆叠状态（$2N$），两阶段行同样各自独立。物理时间步长在两种模式下均为全局标量（逐 DOF 物理步长属于局部时间步进，不在当前范围内）。
