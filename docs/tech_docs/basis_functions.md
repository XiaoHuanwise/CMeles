...
> **备注**：模态到节点的求值在数学上等价于对基函数的双重求和，同样可应用求和分解（见 2.5 节）拆分为两次一维操作。但 Vandermonde 矩阵-向量乘法形式 $\mathbf{u} = \mathbf{V}_{2D} \hat{\mathbf{u}}$ 表达更加简洁，且可直接调用高度优化的 BLAS 库，在缓存利用和并行化方面通常优于手写的求和分解循环。因此，数值积分的双重求和是求和分解的主要应用场景（见 2.5 节；当前体积分实现为稠密收缩，求和分解列为后续优化），而 Vandermonde 求值优先采用矩阵乘法形式。
...
> **备注**：方式一和方式二在数学上完全等价，计算量相同。但在 $\mathcal{P}_N$ 空间中，每层 $i$ 对应的 $j$ 数量不同（$N - i + 1$），方式二的分组循环长度不规则，优化收益有限。实现求和分解时拟采用方式一，以展平索引 $\ell$ 为主线。
>
> **复杂度前提**：实现本节开头的降阶（$O(N_q^2 N + N^2 N_q)$），要求第一步的中间量 $\tilde{G}$ 按**一维下标 $j$**（共 $N+1$ 个）索引，并在所有满足 $j_\ell = j$ 的基函数 $\ell$ 之间复用。若按 $\ell$ 逐一计算并存储 $N_{\text{base}} \times N_q$ 的中间数组，第一步代价为 $N_{\text{modes}} N_q^2$，与直接稠密收缩同阶，降阶收益为零。当前体积分核函数采用稠密收缩（见[控制方程与 DG 场文档](governed_equations_and_DG_field.md#二体积分计算)），求和分解列为后续优化方向。
