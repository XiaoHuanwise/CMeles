...
| 文件 | 内容 |
| ---- | ---- |
| `TimeTypes.hpp` | `RhsFunction` 类型别名 |
...
- 唯一的运行期分派点是 time 模块的 `makeStepperFactory(const Config&)`
  （`src/time/StepperFactory.cpp`），按配置返回具体 stepper 的工厂；
  `runCompressibleFlowSolver` 仅构造非模板的求解器控制器（单一实例化，实现在 .cpp）并运行，
  新增时间格式只需改动 time 模块。
- 时间步长：`DgField::estimateDt` 按本文档 CFL 公式每步重算（`estimate_dt.okl`），
  `[time_marching] dt > 0` 时使用固定步长。
- 单元测试见 `ctest/time/test_time_stepper.cpp`（ODE $u' = \lambda u$ 收敛阶）、
  `ctest/solver/test_euler_vortex.cpp`（SSPRK3 等熵涡整链路）与
  `ctest/solver/test_vortex_methods.cpp`（其余全部方法的精度/收敛阶，经由生产工厂分派）。
