...
/// @brief Unified OCCA memory management, abstracting backend memory model differences.
///
...
public:
    ...
    ///
    /// @param src     Host source pointer (may be `nullptr` for uninitialised allocation).
    ...
    /// Otherwise no-op (same address space).
    void copyFromHost(occa::memory &o_data, const Real *src, occa::dim_t entries);
