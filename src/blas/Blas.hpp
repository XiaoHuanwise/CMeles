...
public:
    ...
    /// @param tileSize New tile size (must be a compile-time constant for @tile).
    ...
    void gemv(occa::dim_t m, occa::dim_t n, Real alpha,
              occa::memory &A, occa::memory &x, Real beta, occa::memory &y);

    /// A = alpha * x * y^T + A
    /// @param A m×n row-major matrix (in/out)
    void ger(occa::dim_t m, occa::dim_t n, Real alpha,
