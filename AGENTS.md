...
- **公式不得跨行**：每个公式必须连同定界符完整写在同一行注释内。Comment Formula 插件（默认配置）捕获跨行 `$$ ... $$` 时会把中间各行的 `//` 前缀吞入公式导致渲染出错；跨行的 `$ ... $` 则被直接忽略、不渲染
- **定界符必须严格配对**：`$$` 开始的公式必须以 `$$` 结束，不得与单个 `$` 混用；禁止使用 `\[ ... \]`、`\( ... \)` 等插件无法识别的定界符
- **行间公式前后需空行**：`$$ ... $$` 渲染后高度可能超过一行，公式行必须独占一行注释，且其前一行与后一行必须是空注释行（如单独的 `///`），避免渲染结果与相邻文字重叠
- 长公式允许超出 80 列行宽：`.clang-format` 已设置 `ReflowComments: false`，clang-format 不会折行注释。编写时宁可单行超长也不要手动折行公式；`\begin{aligned}` 等多行环境可出现在公式内部，但整个 `$$ ... $$` 仍须在单行内
